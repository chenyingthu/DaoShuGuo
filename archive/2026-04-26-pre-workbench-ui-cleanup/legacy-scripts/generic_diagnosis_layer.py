...
ROUTING_POLICY: dict[str, dict[str, Any]] = {
    ...
    "skill_structure_problem": {
        "workers": {"skill_worker", "cognition_worker", "human_review"},
        "continue_loop": True,
        "actions": {"continue_skill_evolution", "redesign_skill_structure", "repair_skill", "human_review"},
        "allow_stop_actions": {"repair_skill"},
    },
    "evaluator_design_problem": {
    ...
    "evidence_insufficiency": {
        "workers": {"cognition_worker", "literature_worker", "human_review"},
        "continue_loop": False,
        "actions": {"request_literature_alignment", "pause_for_review", "human_review"},
    },
    "research_value_problem": {
        "workers": {"portfolio_worker", "human_review"},
        "continue_loop": False,
        "actions": {"pause_or_stop", "human_review"},
    },
}
...
def build_diagnosis_input(
    *,
    ...
    task_adapter_checks = {
        "has_baseline_binding": bool(
            task_adapter.get("baseline_binding", {}).get("baseline_refs") or task_adapter.get("baseline_ref")
        ),
        "has_candidate_binding": bool(
            task_adapter.get("candidate_binding") or task_adapter.get("candidate_skill_refs")
        ),
        "has_evaluator_binding": bool(
            task_adapter.get("evaluator_binding", {}).get("evaluator_ref") or task_adapter.get("evaluator_ref")
        ),
        "has_search_envelope": bool(task_adapter.get("search_envelope") or task_adapter.get("metrics_mapping")),
        "has_diagnosis_hook_config": bool(task_adapter.get("diagnosis_hook_config") or task_adapter.get("claim_gates")),
    }
    required_refs = {
...
def validate_diagnosis_fields(fields: dict[str, Any]) -> list[str]:
    ...
    if problem_class in ROUTING_POLICY:
        ...
        allow_stop = (
            fields.get("recommended_action") in policy.get("allow_stop_actions", set())
            and fields.get("continue_loop") is False
        )
        if (
            isinstance(fields.get("continue_loop"), bool)
            and fields["continue_loop"] != policy["continue_loop"]
            and not allow_stop
        ):
            issues.append(
