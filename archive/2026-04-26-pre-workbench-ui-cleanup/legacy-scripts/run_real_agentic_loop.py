...
def normalize_confidence(value: Any) -> str:
    """Map free-form LLM confidence labels into the schema enum."""
    ...
    if normalized in {"low", "medium", "high"}:
        return normalized
    if normalized in {"medium_high", "moderate_high", "medium_to_high"}:
        return "high"
    if normalized in {"low_medium", "medium_low", "moderate"}:
        return "medium"
    return "medium"
...
def build_agentic_update(iteration: int, request: dict[str, Any], result: dict[str, Any], run_obj: dict[str, Any], workflow_refs: list[str]) -> Path:
    ...
    payload = {
        ...
        "summary": summary,
        "confidence": normalize_confidence(adjudicator.get("confidence", "medium")),
    }
