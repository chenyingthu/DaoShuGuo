...
This module is backend-agnostic at the worker-contract level. The loop engine
injects `backend_config`; this module chooses the configured runner and asks it
to author phase-local worker JSON. The controller remains responsible for
persistence, verification, and routing.
"""
...
""".strip()
    ...
    for attempt in range(1, attempts + 1):
        ...
        if run["exit_code"] != 0:
            ...
            continue
        text = _assistant_text(run["events"]) or run["stdout"]
        try:
            ...
            prompt = (
                ...
                "Return only a compact complete JSON object now. "
                "No Markdown, no prose, no trailing explanation.\n\n"
        ...
        prompt = (
            ...
            "Return only a corrected JSON object. Keep your research judgment, but express it using the allowed "
            "contract vocabulary exactly. Do not include Markdown or prose.\n\n"
