#!/usr/bin/env python3
"""Verify the stabilized step-based Pi task003 loop artifacts."""
...
ROOT = REPO_ROOT / "analysis" / "pi_harness" / "pi_json_loop_task003_state"
...
def main() -> int:
    ...
    needed = ["init_research_task", "task003_trial", "skill_trial", "cognition_constraint", "iteration_review"]
    for name in needed:
        if not any(f'"event":"{name}"' in line for line in entries):
            raise RuntimeError(f"missing event in research_loop.jsonl: {name}")

    print("Pi task003 state-loop verification passed.")
    return 0
